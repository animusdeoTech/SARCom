...
use crate::ui::{format_age, palette::{HEADER_BG, SIDEBAR_BG, TEXT_BRIGHT, MAP_BG}};
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        if self.show_edit {
            ...
            egui::Window::new("Edit / Tweak")
                ...
                .default_width(260.0)
                .resizable(true)
                .show(ctx, |ui| { self.show_edit_panel(ui, t); });
            self.show_edit = open;
        }
        ...
        egui::TopBottomPanel::top("header")
            ...
            .show(ctx, |ui| { self.show_header(ui, t); });

        // SOS banner (conditionally shown)
        let sos_tags: Vec<&TagData> = self.sim.tags.iter().filter(|t| t.sos).collect();
        if !sos_tags.is_empty() {
            let label = sos_tags[0].label.clone();
            let age   = format_age(sos_tags[0].last_seen_secs);
            egui::TopBottomPanel::top("sos_banner")
                ...
                .show(ctx, |ui| {
                    ui.horizontal(|ui| {
                        ui.label(egui::RichText::new("⚠  SOS ALERT")
                            .color(egui::Color32::WHITE).strong().size(13.0));
                        ui.separator();
                        ui.label(egui::RichText::new(&label).color(egui::Color32::WHITE).strong());
                        ui.label(egui::RichText::new(&age)
