...
enum WayKind { HighwayMajor, HighwayMinor, Water, Forest }
...
impl OsmMap {
    ...
    pub fn draw(&self, painter: &egui::Painter, rect: egui::Rect) {
        ...
        let to_pt  = |id: &i64| -> Option<egui::Pos2> {
            nodes.get(id).map(|(lat, lon)| bounds.to_screen(*lat, *lon, rect))
        };
        ...
        for way in self.ways.iter().filter(|w| !w.closed && w.kind == WayKind::Water) {
            let pts: Vec<egui::Pos2> = way.refs.iter().filter_map(|id| to_pt(id)).collect();
            if pts.len() < 2 { continue; }
            let s = egui::Stroke::new(1.5, egui::Color32::from_rgba_unmultiplied(50, 120, 200, 170));
        ...
        for way in self.ways.iter().filter(|w| w.kind == WayKind::HighwayMajor) {
            let pts: Vec<egui::Pos2> = way.refs.iter().filter_map(|id| to_pt(id)).collect();
            if pts.len() < 2 { continue; }
            let s = egui::Stroke::new(1.5, egui::Color32::from_rgba_unmultiplied(120, 120, 120, 190));
            for w in pts.windows(2) { painter.line_segment([w[0], w[1]], s); }
        }
...
fn classify(tags: &[(String, String)]) -> Option<WayKind> {
    for (k, v) in tags {
        match k.as_str() {
            "highway" => return Some(match v.as_str() {
                "primary" | "secondary" | "tertiary" | "residential" | "living_street" => WayKind::HighwayMajor,
...
fn parse(xml: &str) -> OsmMap {
    let mut bounds = Bounds { min_lat: 0.0, max_lat: 0.0, min_lon: 0.0, max_lon: 0.0 };
        ...
        match event {
            XmlEvent::StartElement { name, attributes, .. } => {
                let get = |key: &str| -> Option<&str> {
                    attributes.iter().find(|a| a.name.local_name == key).map(|a| a.value.as_str())
                };
                match name.local_name.as_str() {
                    "bounds" => {
                        let p = |k| get(k).and_then(|v| v.parse::<f64>().ok()).unwrap_or(0.0);
                        bounds = Bounds {
                            min_lat: p("minlat"), max_lat: p("maxlat"),
                            min_lon: p("minlon"), max_lon: p("maxlon"),
                        };
                    }
                    "node" => {
                    ...
                    "way" => { in_way = true; cur_refs.clear(); cur_tags.clear(); }
                    "nd" if in_way => {
            ...
            XmlEvent::EndElement { name } if name.local_name == "way" && in_way => {
                ...
                if let Some(kind) = classify(&cur_tags) {
                    if cur_refs.len() >= 2 {
                        ...
                        ways.push(Way { refs: cur_refs.clone(), kind, closed });
                    }
                }
            }
            _ => {}
