...
use crate::ui::palette::{AMBER, GREEN, ORANGE, TEXT_DIM};
use eframe::egui;
...
impl KioskLabApp {
    pub(crate) fn show_header(&mut self, ui: &mut egui::Ui, t: f64) {
        ui.horizontal(|ui| {
            ...
            // ADR-011: when RTC is unset at boot the operator must see this immediately.
            if !self.sim.clock_valid {
                ui.label(
                    egui::RichText::new("⚠ RTC NOT SET")
                        .color(AMBER)
                        .strong()
                        .size(11.0),
                );
            ...
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                if self.sim.clock_valid {
                    ...
                    ui.label(
                        egui::RichText::new("CLOCK INVALID")
                            .color(AMBER)
                            .monospace()
                            .strong()
                            .size(10.0),
