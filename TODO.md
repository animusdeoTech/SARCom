...
- [ ] Write the `protocol` crate: ONE packet type — **POSITION** (16 B payload / 22 B frame, `seq_nr: u32 BE`, layout per [ARCHITECTURE.md §7](ARCHITECTURE.md) / [ADR-013](decisions/ADR-013-multi-hop-flood-via-packet-id.md) §3). **CRC-16/CCITT-FALSE** (poly 0x1021, init 0xFFFF, no reflect, xorout 0x0000; check value 0x29B1 for "123456789"). One canonical hex test vector frozen in the crate before firmware writes against the wire (`node_id=1`, `seq_nr=42`, `flags.GPS_VALID=1`, `lat=47.1234567`, `lon=13.5678901`, `alt=1847`); plus a second canonical vector with `flags.SOS=1` to lock the distress-bit encoding. SOS flag semantics with jittered-cadence + immediate-first-frame + duty-cycle-cap unit tests in the `tag` crate per [ADR-010](decisions/ADR-010-sos-encoding.md).
