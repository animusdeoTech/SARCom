...
- **Firmware / gateway / UI language: Rust, end to end.** Tag firmware, relay firmware, gateway receiver, kiosk UI — all Rust. No C, no C++, no Zephyr, no Python runtime, no JavaScript, no TypeScript, no React, no npm. See [ADR-001](decisions/ADR-001-firmware-language.md).
...
- **Relay GNSS:** on the board (UC6580), used only during commissioning and maintenance. OFF during normal forwarding. BLE maintenance CLI is v2+, not v1. See [ADR-006](decisions/ADR-006-relay-has-gnss.md).
...
- **Gateway time source:** DS3231 I²C RTC module with CR2032 coin cell is primary; Dragino HAT MTK3339 GPS/PPS is opportunistic. No NTP. See [ADR-011](decisions/ADR-011-gateway-time-source.md).
