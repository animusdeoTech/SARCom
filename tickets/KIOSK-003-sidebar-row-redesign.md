...
- **Collapse each row to one primary line.** Row format is **uniform across node kinds** — per `dev-log/2026-05-19-v1a-ui-data-model-collapse-nodedata.md`, the UI data model is one `NodeData` shape; the only kind-distinction is icon glyph + colour from the inventory map (`HashMap<u8, NodeKind>`). Rows differ by node **state** (POSITION-derived), not by node kind:
  - Fresh: `● {label}  ·  {age}` — icon + label colour from inventory.kind (tag=BLUE / relay=ORANGE / gw=GREEN), state-bullet colour from `freshness_color(Fresh)` per [`tools/sarcom-kiosk-lab/src/map/markers.rs:40-48`](../tools/sarcom-kiosk-lab/src/map/markers.rs).
  - Aging: `● {label}  ·  {age}` — state bullet colour from `freshness_color(Aging)`.
  - Stale: `● {label} · stale · {age}` — `TEXT_DIM`.
  - Very-stale: `● {label} · very stale · {age}` — `TEXT_DIM` (more muted).
